...
beforeAll(() => {
    ...
    return storage.listObjects('bucket-001').then(fs => {
        ...
        return Promise.all([
            ...
            console.log('Let the delete operation really done!')
            return timeout('32s')
        })
        */
    }).catch(ex => { })
...
test('create buckets', function () {
    return storage.ensureBucket('bucket-001', {}).then(() => {
        return storage.hasBucket('bucket-001').then(r => {
            expect(r).toBeTruthy()
            return storage.hasBucket('bucket-xxx')
        }).then(r => {
            expect(r).toBeFalsy()
            return storage.ensureBucket('bucket-002').then(r => storage.hasBucket('bucket-002'))
        }).then(r => {
            expect(r).toBeTruthy()
        })
    })
...
test('list buckets', function () {
    return storage.listBuckets().then(rs => {
        expect(rs).toEqual(expect.arrayContaining(['bucket-001', 'bucket-002']))
    })
}, 1000 * 60)
