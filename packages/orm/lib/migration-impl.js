// Modified from https://github.com/flexxnn/sequelize-auto-migrations
...
const reverseModels = function (sequelize, models) {
    ...
    for (let model in models) {
        if ('default' === model) continue
        let attributes = _.cloneDeep(models[model].attributes || models[model].rawAttributes)
        for (let column in attributes) {
            ...
            for (let property in attributes[column]) {
                ...
                if (property === 'defaultValue') {
                    ...
                    if (_val.notSupported) {
                        $logger.warn(`[Not supported] Skip defaultValue column of attribute ${model}:${column}`);
                        delete attributes[column][property];
                        continue;
                    }
            ...
            if (typeof attributes[column]['type'] === 'undefined') {
                if (!attributes[column]['seqType']) {
                    $logger.warn(`[Not supported] Skip column with undefined type ${model}:${column}`);
            ...
            if (seqType === 'Sequelize.VIRTUAL') {
                $logger.warn(`[SKIP] Skip Sequelize.VIRTUAL column "${column}"", defined in model "${model}"`);
                continue;
            }
        ...
        tables[models[model].tableName].indexes = hashedIndexes = {}
        let indexes = _.cloneDeep(models[model].options.indexes)
        if (indexes.length > 0) {
            for (let _i in indexes) {
                let index = parseIndex(indexes[_i]);
                hashedIndexes[index.hash + ''] = index;
                delete index.hash;
                Object.freeze(index);
            }
        }
...
const parseDifference = function (previousState, currentState) {
    ...
    for (let _d in difference) {
        ...
        switch (df.kind) {
            ...
            case 'A':
                {
                    $logger.warn("[Not supported] Array model changes! Problems are possible. Please, check result more carefully!");
                    $logger.warn("[Not supported] Difference: ");
                    $logger.warn(JSON.stringify(df, null, 2));
                }
                break;
...
const writeMigration = function (currentState, migration, migrationsDir, name) {
    let revision = currentState.revision
    let fullname = _.padStart(revision + '', 3, '0') + `-${name.replace(' ', '_')}`
    let info = {
        revision,
        name,
        file: fullname,
    ...
    let commands = _.map(migration.commandsUp, function (upStr) {
        eval(`var up=${upStr}`)
        let p1 = upStr.indexOf('params:')
        p1 = upStr.indexOf('[', p1)
        let p2 = upStr.lastIndexOf(']')
        let args = _.trim(upStr.substring(p1 + 1, p2))
        return `
        log && log(\`Processing ${up.fn}(${args.indexOf('\n') == -1 ? args : args.substring(0, args.indexOf('\n')) + '...'} )\`)
        await query.${up.fn}(${args})`
    })
    let template = `
const Sequelize = require("sequelize")
module.exports = {
...
`
    fs.writeFileSync(path.join(migrationsDir, fullname + '.js'), template)
    fs.writeFileSync(path.join(migrationsDir, fullname + '.json'), JSON.stringify(currentState, null, 2))
    return info
}
